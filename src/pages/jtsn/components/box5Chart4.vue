...
export default {
    ...
    methods: {
        ...
        fetchData () {
            const url = '/api/terminal/getTerminalPassengerTrend'
            const data = {
            ...
            getData(url, data).then((res) => {
                ...
                for (const datum of res.data) {
                    // debugger
                    sData1.push(this.utils.tofixed(datum.leaveValue/10000,4))
                    sData2.push(this.utils.tofixed(datum.arrivalValue/10000,4))
                    let dss = datum.statDate.slice(4)
                    let temp1 = dss.slice(0,2)
                    let temp2 = dss.slice(2,4)
                    xData.push(temp1 + '-' + temp2)
                }
                // debugger
                this.initChart1(sData1,sData2, xData)
            })
        },
        initChart1 (sData1,sData2,xData) {
            // debugger
            if (!this.chart1) {
