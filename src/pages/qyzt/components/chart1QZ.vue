...
export default {
    ...
    methods: {
        ...
        getData () {
            ...
            const url = '/qz/qzFlowStayRealTime'
            const data = {
                // date: '2019-12-10'
                date: this.checkDate
            }
            postData(url, data).then((res) => {
                ...
                // }
                let start = 0
                for (let i = start; i < res.data.length; i++) {
                    let datum = res.data[i];
                    sData.push(this.utils.tofixed(datum.qzStayPercentage*100,2))
                }
                for (var i = start; i < 8; i++) {
                    xData.push(this.zhuliuMap[i])
                }
                // debugger
