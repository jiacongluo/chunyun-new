...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async initChart() {
                ...
                var option = {
                    ...
                    geo3D: {
                        ...
                        // }
                        viewControl: {
                            "alpha": 44.34366905742492,
                            "beta": -16.709289189074873,
             ...
             */
            async initChartBase() {
                let theData = await  resUtil.getRes("map/province/guangdong.json", {});
                var me = this;

                if (theData == null) {
                    console.log("获取地图数据失败!");
                }
                echarts.registerMap("广东", theData);
            ...
            async loadData(dataArgs) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    var theGpsFromPoint = GpsUtil.getByCityName(theFromName);
                    if (!theGpsFromPoint) {
                        console.log('未找到坐标:' + theFromName);
                        continue;
                    }
                    var theGpsToPoint = GpsUtil.getByCityName(theToName);
                    if (!theGpsToPoint) {
                ...
                // this.chartView.getZr().on('mousedown', function (params) {
                //     var theOption = me.chartView.getOption();
                //     // debugger;
