export default {
    getProvinceCode(name) {
        ...
        var theMap={"广西":"广西壮族自治区"};
        var name=theMap[name]||name;
    ...
    getCityCode(provinceName, cityName) {
        var theCode = this.getProvinceCode(provinceName);
        if (theCode) {
    ...
    getCountyCode(provinceName, cityName, countyName) {
        var theCityCode = this.getCityCode(provinceName, cityName);
        if (theCityCode) {
    ...
    //获取区域列表
    getGdAreaList(cityCode,ori) {
        var theResultList = [];
        var theCode=cityCode.substr(0,4);
        for (var i = 0; i < this.gd.length; i++) {
            var theItem = this.gd[i];
            if(theItem.code.startsWith(theCode)&&theItem.code.endsWith("00")==false){
                if (ori) {
                    theResultList.push({
                        name:theItem.name.substr(0,theItem.name.length-1),
                        value:theItem.name.substr(0,theItem.name.length),
                    });
                } else {
                    theResultList.push({
                        name:theItem.name.substr(0,theItem.name.length-1),
                        value:theItem.name.substr(0,theItem.name.length-1),
                    });
                }

                // theItem.value=theItem.name.substr(0,theItem.name.length-1);
                // theItem.name=theItem.value;
            }
        }
        return theResultList;
    },
    //获取区域列表
    getGdAreaSelectList(cityCode) {
        var theResultList = [];
        var theCode=cityCode.substr(0,4);
        var thePrex="";
        ...
        for (var i = 0; i < this.gd.length; i++) {
            ...
            if(theItem.code.startsWith(theCode)){
                if(theItem.code.endsWith("00")==false){
                    theResultList.push({
                        name:theItem.name.substr(0,theItem.name.length-1),
                        value:thePrex+theItem.name,
                    });
                }
                else{
                    // debugger;
                    thePrex=theItem.name.substr(0,theItem.name.length-1);
                }
